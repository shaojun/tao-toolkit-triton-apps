...
class BlockingDoorEventDetector(EventDetectorBase):
    ...
    def detect(self, filtered_timeline_items: List[TimelineItem]):
        ...
        return EventAlarm(EventAlarmPriority.ERROR, "detected BlockingDoorEvent")
...
class ElectricBicycleEnteringEventDetector(EventDetectorBase):
    ...
    def detect(self, filtered_timeline_items: List[TimelineItem]):
        ...
        last_handled_original_timestamp = datetime.datetime(1999, 1, 1, 0, 0)
        for item in filtered_timeline_items:
            item.consumed = True
            current_original_timestamp = datetime.datetime.fromisoformat(
                item["original_timestamp"].replace("Z", "+00:00"))
            if (current_original_timestamp - last_handled_original_timestamp).total_seconds() \
                    <= minimal_handle_interval_by_second:
                continue
            last_handled_original_timestamp = datetime.datetime.fromisoformat(
                item["original_timestamp"].replace("Z", "+00:00"))
            sections = item.split('|')
            ...
            print("(localConf:{})infer_results: {}".format(edge_board_confidence, infer_results))
